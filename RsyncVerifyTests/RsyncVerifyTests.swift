...
struct RsyncAnalyzerTests {
    ...
    func statisticsWithCommas() async {
        ...
        #expect(result?.statistics.totalFileSize == 1024576)
...
struct RsyncFileChangeTests {
    ...
    func multipleAttributes() {
        ...
        let attributeNames = change?.attributes.map { $0.name } ?? []
...
struct IntegrationTests {
    ...
    func endToEndIntegration() async {
        ...
        *deleting manually_deleted.txt
        ...
        // Verify data sizes
        #expect(result?.statistics.totalFileSize == 10485760)
        #expect(result?.statistics.totalTransferredSize == 2621440)
        #expect(result?.statistics.literalData == 1048576)
        #expect(result?.statistics.matchedData == 1572864)
    }
}
